...
from trend_score.compare import compare_two_waves, rank_waves, score_interval_continuity
...
def _render_asset_group(st, px, go, data_dir: Path, group: str, min_reversal: float | None) -> None:
    ...
    _render_wave_compare(st, px, filtered_df, ranked)
...
def _render_price_chart(st, go, df: pd.DataFrame, waves: pd.DataFrame) -> None:
    ...
    for _, wave in waves.iterrows():
        color = "rgba(22, 163, 74, 0.14)" if wave["direction"] == "up" else "rgba(220, 38, 38, 0.14)"
        fig.add_vrect(
            x0=wave["start_date"],
            x1=wave["end_date"],
            fillcolor=color,
    ...
    fig.update_layout(height=420, margin={"l": 20, "r": 20, "t": 40, "b": 20}, title="收盘价与波段标注")
...
def _render_wave_detail(st, px, waves: pd.DataFrame) -> None:
    ...
    right.plotly_chart(px.bar(dims, x="维度", y="得分", range_y=[0, 100], title="维度拆解"), use_container_width=True)
...
def _render_wave_compare(st, px, df: pd.DataFrame, waves: pd.DataFrame) -> None:
    ...
    if len(waves) < 2:
        ...
        return
    labels = [_wave_label(index, row) for index, row in waves.iterrows()]
    selected = st.multiselect("选择两个波段", labels, default=labels[:2], max_selections=2)
    if len(selected) != 2:
        st.info("请选择两个波段。")
        return
    first_index = int(selected[0].split(" | ", 1)[0])
    second_index = int(selected[1].split(" | ", 1)[0])
    comparison = compare_two_waves(df, waves, first_index, second_index)
    st.plotly_chart(
        px.line(comparison["paths"], x="step", y="relative_close", color="wave", title="归一化走势（起点=100）"),
        use_container_width=True,
    )
    st.dataframe(_format_dates(comparison["metrics"]), use_container_width=True, hide_index=True)
...
def _render_interval_continuity(st, px, data_dir: Path, group: str, symbol_files: dict[str, Path]) -> None:
    ...
    st.plotly_chart(
        px.bar(chart_df, x="interval_score", y="label", orientation="h", title="区间连续性评分排名", range_x=[0, 100]),
        use_container_width=True,
    )
